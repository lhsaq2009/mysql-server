...
enum enum_mdl_type {
    /*
      元数据意向独占锁；仅适用于带范围 ( 作用域 ) 锁
      An intention exclusive metadata lock. Used only for scoped locks.
      此类锁的所有者，可以在单个对象上获取可升级的独占锁
      Owner of this type of lock can acquire upgradable exclusive locks on individual objects.

      在字典缓存中执行查找时，也使用此锁定类型
      This lock type is also used when doing lookups in the dictionary cache.
      ✅ 在 schema 获取对象时，我们使用 IX 锁定 schema 以防止 schema 被删除
      When acquiring objects in a schema, we lock the schema with IX to prevent the schema from being deleted.
      从概念上讲，这应该是一个 IS 锁，但它的行为与当前的 IX 相同
      This should conceptually be an IS lock, but it would have the same behavior as the current IX.
      与其它 IX 锁兼容，但与作用域内的 S 和 X 锁不兼容
      Compatible with other IX locks, but is incompatible with scoped S and X locks.
    */
    MDL_INTENTION_EXCLUSIVE = 0,    // GLOBAL 对象、SCHEMA 对象操作会加此锁
    /*
      共享元数据锁；A shared metadata lock.
      用于我们只对 object metadata 感兴趣 并且 无意访问 object data 的情况 ( 例如，用于存储例程 或 在准备预准备语句期间 )
      To be used in cases when we are interested in object metadata only and
      there is no intention to access object data (e.g. for stored routines or during preparing prepared statements).

      我们还将这种类型的锁，滥用于打开处理程序，因为此语句获取的锁 必须与 LOCK TABLES ... WRITE 获取的 表写锁 兼容 ，
      例如，TODO 2023-05-24：SNRW ？？？（ 我们无法通过在 HANDLER 处获取 S 锁来过日子......打开时间
      并将其升级到处理程序的 SR 锁...READ，因为它不能解决需要中止 DML 语句的问题，这些语句在表级锁定上等待，同时在同一连接中打开 HANDLER）。

      We also mis-use this type of lock for open HANDLERs,
      since lock acquired by this statement has to be compatible with lock acquired by LOCK TABLES ... WRITE statement,
      i.e. SNRW (We can't get by by acquiring S lock at HANDLER ... OPEN time and upgrading it to SR
                                   因为它不能解决需要中止 DML 语句的问题，在同一连接中打开 HANDLER 时等待表级锁定
      lock for HANDLER ... READ as it doesn't solve problem with need to abort DML statements
      which wait on table level lock while having open HANDLER in the same connection).

      为了避免在将 SNRW 锁升级到 X 锁时可能发生的死锁，因为该表上有一个活动的 S 锁，该锁由线程拥有，
      该线程依次等待执行升级的线程拥有的表级锁，在这种情况下，我们必须使用 thr_abort_locks_for_thread() 工具。

      存储例程上的锁不会出现此问题，因为我们不对它们使用 SNRW 锁。在 PREPARE 调用期间使用 S 锁时也不会出现此问题，因为在此未获取表级锁 箱。
      ...
      case.
    */  // CREATE TABLE 中创建的表上的锁 “S”
    MDL_SHARED,                     // FLUSH TABLES with READ LOCK;
    /*
      高优先级的共享元数据锁
      A high priority shared metadata lock.

      用于无意访问 对象数据 ( 即表中的数据 ) 的情况。
      Used for cases when there is no intention to access object data (i.e. data in the table).

      “高优先级” 意味着，与其它共享锁不同，它可以 忽略 Pending 独占锁的请求 而被授予。
      适用于我们只需要访问 元数据 而不是数据的情况，例如填充 INFORMATION_SCHEMA 表时。
      "High priority" means that, unlike other shared locks, it is granted
      ignoring pending requests for exclusive locks. Intended for use in
      cases when we only need to access metadata and not data, e.g. when filling an INFORMATION_SCHEMA table.

      由于「SH 锁」与「SNRW 锁」兼容，因此 conn 持有「SH 锁」，不应该再尝试获取任何类型的表锁或行锁，因为这可能会导致死锁。
      而且，之后获取「SH 锁」后，连接不应等待任何其它资源，因为它可能会导致 X 锁的 "饥饿" 和 将 SNW 或 SNRW 升级到 X 锁期间出现 潜在的死锁
      ( 例如，如果升级连接将保留正在等待的资源 )
    ...
    */
    MDL_SHARED_HIGH_PRIO,           // ‼️️SH，仅对 MyISAM 有效
    /*
      共享元数据锁，用于有意从表中读取数据的情况
      A shared metadata lock for cases when there is an intention to read data from table.

      持有这种锁的连接可以读取 表元数据 并读取 表数据 ( 在获取适当的 表锁和行锁 之后 )。
      这意味着一个人只能获得 TL_READ、TL_READ_NO_INSERT 和 如果表上持有 SR MDL 锁，则表上类似的表级锁

      TODO 2023-05-24：用于 SELECT、子查询、LOCK TABLE ...  READ
      ...
      statements.
    */
    MDL_SHARED_READ,                // SR，SELECT、子查询、LOCK TABLE ... READ
    /*
      共享元数据锁，用于打算修改 ( 而不仅仅是读取 ) 表中数据的情况
      A shared metadata lock for cases when there is an intention to modify (and not just read) data in the table.

      持有 SW 锁的连接，可以 读取表元数据 以及 读写表数据 ( 在获取适当的表和行级锁之后 )
      A connection holding SW lock can read table metadata and modify or read table data (after acquiring appropriate table and row-level locks).

      TODO 2023-05-25：用于要通过 INSERT, UPDATE, DELETE 修改的表，但不包括 LOCK TABLE ... WRITE or DDL； 也包括 SELECT ... FOR UPDATE.

      To be used for tables to be modified by INSERT, UPDATE, DELETE statements,
      but not LOCK TABLE ... WRITE or DDL). Also taken by SELECT ... FOR UPDATE.
    */
    MDL_SHARED_WRITE,               // ✅ SW，INSERT, UPDATE, DELETE，SELECT ... FOR UPDATE
    /*
      MDL_SHARED_WRITE 锁的一个版本，其优先级低于 MDL_SHARED_READ_ONLY 锁
      A version of MDL_SHARED_WRITE lock which has lower priority than MDL_SHARED_READ_ONLY locks.

      由修改表并使用 LOW_PRIORITY 子句的 DML 语句使用
      Used by DML statements modifying tables and using the LOW_PRIORITY clause.
    */
    MDL_SHARED_WRITE_LOW_PRIO,      // ‼️️仅对 MyISAM 有效 ，经验证 innodb 也有
    /*
      可升级的共享元数据锁，允许 并发更新 和 读取表数据
      An upgradable shared metadata lock which allows concurrent updates and reads of table data.

      持有这种锁的连接可以读取 表元数据 和 表数据
      A connection holding this kind of lock can read table metadata and read table data.

      它不应修改数据，因为此锁与 SRO 锁兼容
      It should not modify data as this lock is compatible with SRO locks.

      可升级为 SNW、SNRW 和 X 锁
      Can be upgraded to SNW, SNRW and X locks.

      一旦 SU 锁 升级到 X 或 SNRW 锁，就可以自由修改数据
      Once SU lock is upgraded to X or SNRW lock data modification can happen freely.

      用于 ALTER TABLE 的第一阶段
      To be used for the first phase of ALTER TABLE.
    */
    MDL_SHARED_UPGRADABLE,          // SU，ALTER TABLE .. 的第一阶段
    /*
      共享元数据锁，用于我们需要从表中读取数据的情况，并阻止对其的所有并发修改 ( 针对数据和元数据 )

      A shared metadata lock for cases when we need to read data from table
      and block all concurrent modifications to it (for both data and metadata).

      用于 LOCK TABLES ... READ 语句
      Used by LOCK TABLES READ statement.
    */
    MDL_SHARED_READ_ONLY,           // SRO，用于 LOCK TABLES ... READ
    /*
      可升级的 共享元数据锁，可阻止所有更新表数据的尝试，允许读取
      An upgradable shared metadata lock which blocks all attempts to update table data, allowing reads.

      持有这种锁的连接，可以读取表元数据和读取表数据。可以升级到 X 元数据锁
      A connection holding this kind of lock can read table metadata and read table data.
      Can be upgraded to X metadata lock.

      请注意，由于这种类型的锁与 SNRW 或 SW 锁类型不兼容，因此获取适当的引擎级锁进行读取（ MyISAM TL_READ*，InnoDB 中的共享行锁）应该是无争用的
      Note, that since this type of lock is not compatible with SNRW or SW lock types, acquiring appropriate engine-level locks for reading
      (TL_READ* for MyISAM, shared row locks in InnoDB) should be contention-free.

      TODO 2023-05-25：用于 ALTER TABLE 的第一阶段，在表之间复制数据时，允许表中的并发 SELECT，但不允许 UPDATE
      To be used for the first phase of ALTER TABLE, when copying data between
      tables, to allow concurrent SELECTs from the table, but not UPDATEs.
    */
    MDL_SHARED_NO_WRITE,            // SNW，用于 ALTER TABLE 的第一阶段，在表之间复制数据时，允许表中的并发 SELECT，但不允许 UPDATE
    /*
      可升级的 共享元数据锁，允许「其它连接」访问表元数据，✅ 但不能访问数据
      An upgradable shared metadata lock which allows other connections to access table metadata, but not data.
      它阻止 读取 或 更新 表数据 的所有尝试，TODO 2023-05-25：但允许 INFORMATION_SCHEMA 和 SHOW 查询
      It blocks all attempts to read or update table data, while allowing INFORMATION_SCHEMA and SHOW queries.

      持有这种锁的连接，可以读取表元数据修改和读取表数据
      A connection holding this kind of lock can read table metadata modify and read table data.

      可以升级到「X 元数据锁」，
      Can be upgraded to X metadata lock.

      用于 LOCK TABLES WRITE 语句
      To be used for LOCK TABLES WRITE statement.

      TODO 2023-05-24：与 除 S 和 SH ？？？以外 的任何其它锁类型不兼容
      Not compatible with any other lock type except S and SH.
    */
    MDL_SHARED_NO_READ_WRITE,       // ✅ SNRW，LOCK TABLES ... WRITE
    /*
      元数据独占锁；An exclusive metadata lock.
      持有此锁的连接，可以修改 表的元数据 和 数据。
      A connection holding this lock can modify both table's metadata and data.
      持有此锁时，无法授予其它类型的元数据锁
      No other type of metadata lock can be granted while this lock is held.
      TODO 2023-05-24：用于 CREATE/DROP/RENAME TABLE 以及执行其它 DDL 语句的某些阶段
      To be used for CREATE/DROP/RENAME TABLE statements and for execution of certain phases of other DDL statements.
    */
    MDL_EXCLUSIVE,                  // 用于 CREATE/DROP/RENAME TABLE 以及执行其它 DDL 语句的某些阶段
    /* This should be the last !!! */
    MDL_TYPE_END
};

/** 元数据锁的持续时间；Duration of metadata lock. */

enum enum_mdl_duration {
  /**
    具有 statement 持续时间的锁，会在 statement 或事务结束时自动释放
    Locks with statement duration are automatically released at the end of statement or transaction.
  */
  MDL_STATEMENT = 0,
  /**
    具有事务持续时间的锁，在事务结束时自动释放
    Locks with transaction duration are automatically released at the end of transaction.
  */
  MDL_TRANSACTION,
  /**
   * 具有显式持续时间的锁在语句和事务结束时继续有效。必须通过调用 MDL_context：：release_lock（） 来显式释放它们
...
struct MDL_key {
 public:
  ...
  /**
    命名空间；Object namespaces.
    Sic: when adding a new member to this enum make sure to
    update m_namespace_to_wait_state_name array in mdl.cc!

    不同类型的对象存在于不同的命名空间中；Different types of objects exist in different namespaces
     - GLOBAL       is used for the global read lock.   用于全局读锁定
...
class MDL_request {
  ...
  */
  ...
  /** 这是对强 DDL/LOCK 表类型的锁的请求吗？    Is this a request for a strong, DDL/LOCK TABLES-type, of lock? */
...
/**
  授予的元数据锁；A granted metadata lock.
